  ...
  return (
    ...
    >
      ...
      <div>
        ...
        {fields.map((item, index) => (
          <div key={item.id} className="flex items-center space-x-2 mb-2">
            <Input type="number" placeholder="наприклад: 10" {...register(`values.${index}`, { required: true })} />
      ...
      <div className="flex items-center justify-center">
        <Button size="full">{createDiscountIsLoading ? <IconLoader2 className="animate-spin" /> : "Створити знижку"}</Button>
      </div>
    </form>
  );
};
